...
from base.runner_pose_est_both import train, validate
...
from base.scaphoid_models.PoseEstPointAttN import BothPoseEstPointAttN
...
def main(prio_args=None, prio_config=None) -> torch.Tensor:
    """
    Main function to start one training run of the Pose Estimation Model with Shared Weights between Volar and Dorsal 
    Point Estimator.
    ...
    """
    print_log(f"Main func started with prio_args: {prio_args}, prio_config: {prio_config}", color='red')
    ...
    if config.model_name == 'BothPoseEstPointAttN':
        base_model = BothPoseEstPointAttN(config)
    ...
    if not args.resume:
        b.partly_resume_model_pose_both(base_model, args, config, freeze=False, logger=logger)
    ...
    if args.test:
        ...
        for epoch in range(start_epoch, config.max_epoch + 1):
            print_log(f"################# Epoch {epoch}/{config.max_epoch} ###################", logger, color='blue')
            ...
            if epoch % args.val_freq == 0:
                ...
                if  best_cdl1_score is None or cdl1_score < best_cdl1_score:
                    ...
                    if not args.debug and not args.smac3:
